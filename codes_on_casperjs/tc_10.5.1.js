...
*/
casper.test.begin("Using save icon present in the navbar - Edit R Cell (pre-executed)", 3, function suite(test) {
	...
	casper.viewport(1366,768).then(function(){
		this.sendKeys(x('/html/body/div[3]/div/div[3]/div/div/div[2]/div[3]/div/div/div[2]/div'),"*a");
		this.wait(4000);
	});
	...
	casper.viewport(1366,768).then(function(){
		...
		this.echo(temp);
		this.test.assertNotEquals(temp,code,'Contents of second R cell have got edited and saved');
	});
