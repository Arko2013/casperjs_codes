...
Author: Kunal
Description:This is a casperjs automation script for checking that coalesce Cell option for the topmost cell is not present
*/
casper.test.begin("No Coalesce Cell option for the topmost cell", 4, function suite(test) {
	...
	//Checking whether coalesce button for the top most cell is present or no
	
	casper.viewport(1366,768).then(function(){
		this.wait(15000);
		this.test.assertDoesntExist
		({type: 'xpath', path: '/html/body/div[3]/div/div[3]/div/div/div[2]/div[2]/span/i'},
			'There is no coalesce button/icon for the top-most cell');
	});
