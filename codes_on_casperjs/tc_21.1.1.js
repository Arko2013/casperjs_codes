...
casper.test.begin(" Edit a cell from a notebook", 4, function suite(test) {
   ...
   casper.viewport(1366,768).then(function() {
	...
	//changing the R cell to editable form
	casper.viewport(1366,768).then(function(){
		this.wait(4000);
		if(this.click(x('/html/body/div[3]/div/div[3]/div/div/div/div/div/table/td[2]/span/i')))
		{
			console.log('Cell now in editable form');
		}
		else
		{
			console.log('cell not in editable form');
		}
	});
	
	
	//modify content of the R cell
	casper.viewport(1366,768).then(function(){
		this.sendKeys('div.ace-chrome:nth-child(1) > textarea:nth-child(1)',item2);
		this.wait(5000);
		if(this.thenClick({type:'xpath', path: '/html/body/div[3]/div/div[3]/div/div/div/div/div/table/td/span/i'}))
		{
			this.wait(5000);
   ...
   casper.viewport(1366,768).then(function() {
	   this.wait(3000);
	   this.sendKeys('#input-text-search',item2);
	   this.wait(6000);
   });
   ...
   casper.viewport(1366,768).then(function() {
	   ...
	   for (var i = 1; i <= counter; i++) {
		   ...
		   if(result == combo)
		   {
			   var check = this.fetchText(x('/html/body/div[3]/div/div/div[2]/div/div/div[2]/div[2]/div/div/div[2]/div/div/table['+i+']/tbody/tr[2]/td/div/table/tbody/tr[2]/td/table/tbody/tr/td[2]/code/b'));
	           //this.echo(temp);
	           //this.test.assertEquals(temp,item,'Searched item has been found');
	           if(temp == check )
	           {
				   this.test.assertNotEquals(item1,check,'the item to be searched has been modified');
				   console.log('modified searched item has been found');
			   }//inner if closes
			   else
			   {
