...
casper.test.begin(" Loaded Notebook of some different user (after the Notebook is forked)", 7, function suite(test) {
	...
	casper.viewport(1366,768).thenOpen('http://127.0.0.1:8080/main.html?notebook='+notebook_id,function() {
		...
		this.wait(6000);
	//confirm that main page has got loaded ptoperly by checking one or more elements are present
		this.test.assertExists(
			{type: 'xpath', path: '/html/body/div[2]/div/div[2]/ul/li/span/a' },
			'the element Shareable Link exists'
			);
