/* 
    ...
    Description:    This is a casperjs automated test script for showing that Select the Fork option to add the notebook 
                    to the local user's notebooks. The notebook contents and title should  now be in editable form. 
*/

//Begin Tests

casper.test.begin("Fork Notebook", 12, function suite(test) {
	...
	casper.then(function() {
		...
		var current_name=this.fetchText(x('/html/body/div[2]/div/div[2]/ul/li[6]/a/span'));
		this.sendKeys('#notebook-title','AaBbCc', {keepFocus: true});
		this.sendKeys('#notebook-title', casper.page.event.key.Enter );
		this.wait(7000);
		var new_name=this.fetchText(x('/html/body/div[2]/div/div[2]/ul/li[6]/a/span'));
		this.test.assertNotEquals(new_name,current_name,'Notebook title is editable after being forked');
		this.echo('Notebook forked successfully');
		phantom.exit();
	});
