...
*/
casper.test.begin("Insert a Rmarkdown cell with a Rmarkdown cell", 3, function suite(test) {
	...
	casper.viewport(1366,768).then(function(){
		...
		{
		console.log('New RMarkdown cell created using the plus icon on top right corner of a R cell');
		}
		else
		{
		console.log('No new cell created');
		}
	});
	...
	//Adding a Rmarkdown cell using the plus icon on top of the Rmarkdown cell
	
	casper.viewport(1366,768).then(function(){
		if(this.click({type:'xpath',path:'/html/body/div[3]/div/div[3]/div/div/div/div[2]/span[2]/i'}))
		{
		this.wait(5000);
		console.log('New RMarkdown cell created using the plus icon on top right corner of a Rmarkdown cell');
		}
		else
		{
		console.log('No new cell created');
