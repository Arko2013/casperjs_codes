...
casper.test.begin("Load notebook as Anonymous User", 5, function suite(test) {
    ...
    casper.viewport(1024,768).then(function() {
	 ...
	 casper.viewport(1024,768).then(function() {
		 console.log('clicking on Edit option');
		 this.click(x('/html/body/div[2]/div/div[2]/ul/li/button/i'));
		 this.wait(10000);
	 });
	  ...
	  casper.viewport(1024,768).then(function() {
		 ...
		 this.test.assertUrlMatch(/main*/, 'main.html page has opened'); 
		 this.test.assertTextExists('Please login first!','confirmed that user cannot view notebook in editable form without logging in ');
	  });
