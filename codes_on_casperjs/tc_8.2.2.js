...
*/
casper.test.begin("Delete a Rmarkdown cell and switch notebook", 4, function suite(test) {
	...
	casper.viewport(1366,768).then(function(){
		...
		{
			this.wait(5000);
			console.log('Deleted the mardown cell');
		}
		else
		{	
		console.log('Markdown cell did not get deleted');
	}
	});
	...
	//verify that the deleted markdown cell is not present
	casper.viewport(1024,768).then(function() {
		this.test.assertNotVisible({type: 'css', path: 'div.ace-chrome:nth-child(1) > textarea:nth-child(1)'},'Verified that the deleted markdown cell is not present');
	});
